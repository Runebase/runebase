...
Options:
  -m, --multiprocess     Run multiprocess binaries runebase-node, runebase-gui.
  -M, --monolithic       Run monolithic binaries runebased, runebase-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'runebase-qt [ARGS]' or 'runebase-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'runebased [ARGS]' or 'runebase-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'runebase-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'runebase-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'runebase-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_runebase [ARGS]'.
  chainstate [ARGS] Run runebase kernel chainstate util, equivalent to running 'runebase-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_runebase [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_runebase-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "runebase-gui" : "runebase-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "runebase-node" : "runebased");
        } else if (cmd.command == "rpc") {
            args.emplace_back("runebase-cli");
            ...
            args.emplace_back("runebase-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("runebase-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_runebase");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("runebase-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_runebase");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_runebase-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("runebase-util");
        } else {
