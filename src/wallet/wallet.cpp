...
void CWallet::StakeRunebases(bool fStake)
{
    ::StakeRunebases(fStake, this, stakeThread);
}
...
void CWallet::StartStake()
{
    if(IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
    {
        m_enabled_staking = !m_ledger_id.empty() && RunebaseLedger::instance().toolExists();
    }
    else
    {
