...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " runebase-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The runebase-tx tool is used for creating and modifying runebase transactions.\n\n"
                "runebase-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded runebase transaction, or with \"-create [commands]\" to create a hex-encoded runebase transaction.\n"
                "\n"
                "Usage: runebase-tx [options] <hex-tx> [commands]\n"
                "or:    runebase-tx [options] -create [commands]\n"
                "\n";
