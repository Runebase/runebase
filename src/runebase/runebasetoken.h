#ifndef RUNEBASETOKEN_H
#define RUNEBASETOKEN_H
#include <string>
#include <vector>
#include <map>
#include <uint256.h>
...
public:
    RunebaseToken();
    virtual ~RunebaseToken();

    void setRunebaseTokenExec(RunebaseTokenExec* tokenExec);
    ...
    // Static functions
    static bool ToHash160(const std::string& strRunebaseAddress, std::string& strHash160);
    static bool ToRunebaseAddress(const std::string& strHash160, std::string& strRunebaseAddress);
...
private:
    ...
    RunebaseToken(RunebaseToken const&);
    RunebaseToken& operator=(RunebaseToken const&);

private:
    RunebaseTokenData* d;
};

#endif // RUNEBASETOKEN_H
