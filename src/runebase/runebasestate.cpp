...
RunebaseState::RunebaseState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = RunebaseState::openDB(_path + "/runebaseDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

RunebaseState::RunebaseState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}
...
ResultExecute RunebaseState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, RunebaseTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    newAddress = _t.isCreation() ? createRunebaseAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
...
std::unordered_map<dev::Address, Vin> RunebaseState::vins() const // temp
{
...
void RunebaseState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* RunebaseState::vin(dev::Address const& _a) const
{
    return const_cast<RunebaseState*>(this)->vin(_a);
}

Vin* RunebaseState::vin(dev::Address const& _addr)
{
...
// void RunebaseState::commit(CommitBehaviour _commitBehaviour)
// {
...
void RunebaseState::kill(dev::Address _addr)
{
...
void RunebaseState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address RunebaseState::createRunebaseAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void RunebaseState::deleteAccounts(std::set<dev::Address>& addrs){
...
void RunebaseState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void RunebaseState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
