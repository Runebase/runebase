#include <runebase/runebaseDGP.h>
...
void RunebaseDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool RunebaseDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule RunebaseDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t RunebaseDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t RunebaseDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t RunebaseDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool RunebaseDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void RunebaseDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void RunebaseDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void RunebaseDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void RunebaseDGP::createParamsInstance(){
...
dev::Address RunebaseDGP::getAddressForBlock(unsigned int blockHeight){
...
void RunebaseDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RunebaseDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RunebaseDGP::parseStorageOneUint64(uint64_t& value){
...
void RunebaseDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule RunebaseDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
