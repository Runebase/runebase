...
public:
    ...
    virtual ~RunebaseDelegationPriv()
    {
        if(m_pfDelegations)
...
RunebaseDelegation::RunebaseDelegation():
    priv(0)
{
    priv = new RunebaseDelegationPriv();
}
...
bool RunebaseDelegation::GetDelegation(const uint160 &address, Delegation &delegation, CChainState& chainstate) const
{
...
bool RunebaseDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool RunebaseDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, ChainstateManager &chainman, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> RunebaseDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void RunebaseDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool RunebaseDelegation::ExistDelegationContract() const
{
    LOCK(cs_main);
...
std::string RunebaseDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool RunebaseDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool RunebaseDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool RunebaseDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool RunebaseDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
