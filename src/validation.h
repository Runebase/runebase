...
#include <runebase/storageresults.h>
...
using ExtractRunebaseTX = std::pair<std::vector<RunebaseTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashRunebaseRoot,
    const dev::h256& oldHashStateRoot, const std::vector<RunebaseTransaction>& transactions);
...
class RunebaseTxConverter{

public:

    RunebaseTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionRunebaseTransactions(ExtractRunebaseTX& runebaseTx);
...
private:
    ...
    RunebaseTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<RunebaseTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
