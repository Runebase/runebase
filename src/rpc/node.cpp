    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                auto init = ipc->spawnProcess("runebase-node");
                echo = init->makeEcho();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    RunebaseDGP runebaseDGP(globalState.get(), chainman.ActiveChainstate());

    UniValue obj(UniValue::VOBJ);
    obj.pushKV("maxblocksize", (uint64_t)runebaseDGP.getBlockSize(active_chain.Height()));
    obj.pushKV("mingasprice", (uint64_t)runebaseDGP.getMinGasPrice(active_chain.Height()));
    obj.pushKV("blockgaslimit", (uint64_t)runebaseDGP.getBlockGasLimit(active_chain.Height()));
    ...
    return RPCHelpMan{"getaddressdeltas",
            "\nReturns all changes for an address (requires addressindex to be enabled).\n",
            {
                {"argument", RPCArg::Type::OBJ, RPCArg::Optional::NO, "Json object",
                    {
                        {"addresses", RPCArg::Type::ARR, RPCArg::Optional::NO, "The runebase addresses",
                            {
...
static RPCHelpMan listconf()
{
    return RPCHelpMan{"listconf",
                "\nReturns the current options that runebased was started with.\n",
                {},
                RPCResult{
                    RPCResult::Type::OBJ_DYN, "", "",
                    {
