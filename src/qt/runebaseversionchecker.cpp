...
#define paternVersion "runebase-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

RunebaseVersionChecker::RunebaseVersionChecker(QObject *parent) : QObject(parent)
{
...
bool RunebaseVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> RunebaseVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(RUNEBASE_RELEASES)));
    QEventLoop event;
...
Version RunebaseVersionChecker::getMaxReleaseVersion()
{
