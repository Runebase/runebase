...
Token::Token()
{
    ...
    lstMandatory.append(RunebaseToken::paramAddress());
    lstMandatory.append(RunebaseToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(RunebaseToken::paramSender());
    ...
    lstMandatory.append(RunebaseToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(RunebaseToken::paramAmount());
    lstOptional.append(RunebaseToken::paramGasLimit());
    lstOptional.append(RunebaseToken::paramGasPrice());
    lstOptional.append(RunebaseToken::paramSender());
    lstOptional.append(RunebaseToken::paramBroadcast());
    lstOptional.append(RunebaseToken::paramChangeToSender());
    lstOptional.append(RunebaseToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToRunebaseAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToRunebaseAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
