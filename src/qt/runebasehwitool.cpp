...
class RunebaseHwiToolPriv
{
public:
    RunebaseHwiToolPriv(QObject *parent)
    {
...
RunebaseHwiTool::RunebaseHwiTool(QObject *parent) : QObject(parent)
{
    d = new RunebaseHwiToolPriv(this);
}

RunebaseHwiTool::~RunebaseHwiTool()
{
    delete d;
}

bool RunebaseHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(RunebaseLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(RunebaseLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool RunebaseHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = RunebaseLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(RunebaseLedger::instance().errorMessage());
    return ret;
}

bool RunebaseHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
...
bool RunebaseHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool RunebaseHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool RunebaseHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool RunebaseHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool RunebaseHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = RunebaseLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(RunebaseLedger::instance().errorMessage());
    return ret;
}

bool RunebaseHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = RunebaseLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(RunebaseLedger::instance().errorMessage());
    return ret;
}

bool RunebaseHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
bool RunebaseHwiTool::isStarted()
{
    return d->fStarted;
}

void RunebaseHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool RunebaseHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool RunebaseHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool RunebaseHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool RunebaseHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool RunebaseHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool RunebaseHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
bool RunebaseHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void RunebaseHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
{
    std::string path = RunebaseLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString RunebaseHwiTool::derivationPathP2SH()
{
    std::string path = RunebaseLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString RunebaseHwiTool::derivationPathBech32()
{
    std::string path = RunebaseLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
