...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Runebase
    //   new: C:\Users\Username\AppData\Local\Runebase
    // macOS: ~/Library/Application Support/Runebase
    // Unix-like: ~/.runebase
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Runebase";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Runebase";
#else
    fs::path pathRet;
