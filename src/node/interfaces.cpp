...
public:
    ...
    void getGasInfo(uint64_t& blockGasLimit, uint64_t& minGasPrice, uint64_t& nGasPrice) override
    {
        ...
        blockGasLimit = runebaseDGP.getBlockGasLimit(numBlocks);
        minGasPrice = CAmount(runebaseDGP.getMinGasPrice(numBlocks));
...
public:
    ...
    bool getDelegation(const uint160& address, Delegation& delegation) override
    {
        ...
        return runebaseDelegation.ExistDelegationContract() ? runebaseDelegation.GetDelegation(address, delegation, chainman().ActiveChainstate()) : false;
    }
    bool verifyDelegation(const uint160& address, const Delegation& delegation) override
    {
        return RunebaseDelegation::VerifyDelegation(address, delegation);
    }
