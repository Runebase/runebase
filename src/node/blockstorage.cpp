    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // runebase
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // runebase
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSigDlgt    = diskindex.vchBlockSigDlgt; // runebase
...
bool BlockManager::ReadBlock(Block& block, const FlatFilePos& pos) const
{
    block.SetNull();
