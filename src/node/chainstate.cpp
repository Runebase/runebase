                                    ...
                                    const ChainstateLoadOptions& options)
{
    ...
    const std::string dirRunebase = PathToString(runebaseStateDir);
    ...
    pstorageresult.reset(new StorageResults(PathToString(runebaseStateDir)));
    if (options.reindex) {
...
ChainstateLoadResult VerifyLoadedChainstate(ChainstateManager& chainman, const ChainstateLoadOptions& options)
{
    ...
    globalSealEngine->setRunebaseSchedule(runebaseDGP.getGasSchedule(active_chain.Height() + (active_chain.Height()+1 >= chainman.GetConsensus().QIP7Height ? 0 : 1) ));
