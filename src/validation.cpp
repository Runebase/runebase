                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = runebaseDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            RunebaseTxConverter converter(tx, NULL, NULL, contractflags);
            ...
            for(RunebaseTransaction runebaseTransaction : runebaseTransactions){
                ...
                if(runebaseTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RUNEBASE additional fees, %d delta bytes\n",
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path runebaseDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(runebaseDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // runebase
    RunebaseDGP runebaseDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setRunebaseSchedule(runebaseDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = runebaseDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = runebaseDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = runebaseDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
...
///////////////////////////////////////////////////////////////////////////////////////// runebase
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            RunebaseTxConverter convert(tx, &view, &block.vtx, contractflags);
...
//////////////////////////////////////////////////////////////////
    ...
    uint64_t coinscalc = ((40000000 * COIN) + (((pindex->nHeight - 5000) * 100) * COIN));
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)((40000000 * COIN) + (((pindex->nHeight - 5000) * 100) * COIN))){
