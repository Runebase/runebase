...
dev::eth::EnvInfo ByteCodeExec::BuildEVMEnvironment(){
    ...
    lastHashes.set(tip);
    runebaseutils::HistoricalHashes::instance().set(tip);
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    globalSealEngine->setRunebaseSchedule(runebaseDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = runebaseDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = runebaseDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = runebaseDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= params.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || GetSerializeSize(TX_NO_WITNESS(block)) > dgpMaxBlockSize) // runebase
