...
void checkChainid(dev::h256& hashTx, ChainstateManager& chainman, const std::string& chain, const runebaseutils::ChainIdType& chainId)
{
    // Select params
    ...
    txs.push_back(createRunebaseTransaction(getCode(CodeID::chainIdcontract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...

    {
        ...
        std::vector<RunebaseTransaction> txShanghai;
        txShanghai.push_back(createRunebaseTransaction(getCode(CodeID::getChainId), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
{
    std::vector<RunebaseTransaction> txs;
    txs.push_back(createRunebaseTransaction(getCode(codeId), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...
BOOST_AUTO_TEST_CASE(checking_shanghai_after_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::MAIN, runebaseutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::TESTNET, runebaseutils::ChainIdType::TESTNET);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::REGTEST, runebaseutils::ChainIdType::REGTEST);
...
BOOST_AUTO_TEST_CASE(checking_shanghai_before_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::MAIN, runebaseutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::TESTNET, runebaseutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::REGTEST, runebaseutils::ChainIdType::MAIN);
