...
namespace runebasetxconverter_tests {
//Tests data
...
void checkResult(bool isCreation, std::vector<RunebaseTransaction> results, uint256 hash){
...
void runTest(Chainstate& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    RunebaseTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractRunebaseTX runebaseTx;
    BOOST_CHECK(converter.extractionRunebaseTransactions(runebaseTx));
    std::vector<RunebaseTransaction> result = runebaseTx.first;
    if(script2 == CScript()){
...
using namespace runebasetxconverter_tests;

BOOST_FIXTURE_TEST_SUITE(runebasetxconverter_tests, TestingSetup)
