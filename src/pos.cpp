...
RunebaseDelegation& GetRunebaseDelegation()
{
    static RunebaseDelegation runebaseDelegation;
    return runebaseDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view, Chainstate& chainstate)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        RunebaseDelegation& runebaseDelegation = GetRunebaseDelegation();
        if(!runebaseDelegation.ExistDelegationContract())
        ...
        if(!runebaseDelegation.GetDelegation(address, delegation, chainstate)) {
        ...
        bool verifiedDelegation = runebaseDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee, Chainstate& chainstate)
{
    Delegation delegation;
    RunebaseDelegation& runebaseDelegation = GetRunebaseDelegation();
    bool ret = runebaseDelegation.GetDelegation(address, delegation, chainstate);
    if(ret) ret &= runebaseDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
