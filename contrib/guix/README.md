# Bootstrappable Runebase Core Builds

This directory contains the files necessary to perform bootstrappable Runebase
Core builds.
...
To build Runebase Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Runebase Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/runebase-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/runebase/guix.sigs).
