...
- [Building Runebase Core](#building-runebase-core)
...
Clone the git repositories for runebase and Gitian.
...
```bash
sudo cp /home/debian/runebase/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the runebase repository under 'Fetch and create inputs' to install sources which require
...
To build Runebase Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the runebase repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/runebase/.git/
...
For example:
```bash
URL=https://github.com/laanwj/runebase.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit runebase=${COMMIT} --url runebase=${URL} ../runebase/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit runebase=${COMMIT} --url runebase=${URL} ../runebase/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit runebase=${COMMIT} --url runebase=${URL} ../runebase/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-runebase:
./bin/gbuild --commit runebase=${COMMIT},cpp-eth-runebase=${ETHCOMMIT} --url runebase=${URL},cpp-eth-runebase=${ETHURL} ../runebase/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the runebase git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../runebase/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url runebase=${BTCPATH},signature=${SIGPATH} ../runebase/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/runebase-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/runebase-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/runebase-osx-build.assert
```
