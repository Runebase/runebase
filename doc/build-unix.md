UNIX BUILD NOTES
====================
Some notes on how to build Runebase Core in Unix.
...
This will build runebase-qt as well, if the dependencies are met.
...
memory available when compiling Runebase Core. On systems with less, gcc can be
...
You need to build boost manually, and if it's not in standard library paths, you need to add `/path/to/boost/lib` into `LD_LIBRARY_PATH` env when building Runebase.
...
The package has the following dependencies when used on Ubuntu 16 machine that is not used for building Runebase:

`runebase-qt` require `libxcb-xinerama0` to be installed on Ubuntu 16 (both 32 and 64 bit versions):
```
sudo apt-get install libxcb-xinerama0 -y
```

Runebase require `GCC 7` standard library be installed for Ubuntu 16 only on 32 bit version:
```
...
Security
--------
To help make your Runebase Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Runebase Core should be built with a non-executable stack,
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Runebase Core can
