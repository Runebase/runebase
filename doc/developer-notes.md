...
configuration options and toggled while runebased is running with the `logging`
RPC.  For instance, launching runebased with `-debug` or `-debug=1` will turn on
...
with "play runebases" on a test network.
...
You can then use the `-debug=lock` configuration option at runebased startup or
`runebase-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `runebase-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_runebase
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_runebase --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/runebased -printtoconsole
...
# A coverage report will now be accessible at `./build/test_runebase.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_runebase \
    --object=build/bin/runebased \
...
To profile a running runebased process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep runebased` -- sleep 60
```
...
- [Main thread (`runebased`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `runebase-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `runebase-cli` and the GUI debug console use this table to determine how to
