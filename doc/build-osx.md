...
Runebase Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Runebase Core application using `make deploy`.
...
Runebase Core should now be available at `./src/runebased`.
If you compiled support for the GUI, it should be available at `./src/qt/runebase-qt`.
...
./src/qt/runebase-qt -server # Starts the runebase-qt server mode, allows runebase-cli control
```
