...
This guide describes how to build runebased, command-line utilities, and GUI on macOS.
...
These tools must be installed in order to build Runebase Core from source.
...
Now that all the required dependencies are installed, let's clone the Runebase Core repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone https://github.com/runebase/runebase.git --recursive
```
...
Runebase Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install
...
Run the following in your terminal to compile Runebase Core:
...
``` bash
/Users/${USER}/Library/Application Support/Runebase/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Runebase"

touch "/Users/${USER}/Library/Application Support/Runebase/runebase.conf"
...
```shell
./build/bin/runebased -daemon      # Starts the runebase daemon.
./build/bin/runebase-cli --help    # Outputs a list of command-line options.
./build/bin/runebase-cli help      # Outputs a list of RPC commands when the daemon is running.
./build/bin/runebase-qt -server # Starts the runebase-qt server mode, allows runebase-cli control
```
