...
On Unix systems, the `-DWITH_MULTIPROCESS=ON` build option can be passed to build the supplemental `runebase-node` and `runebase-gui` multiprocess executables.
...
cmake --build build
build/bin/runebase-node -regtest -printtoconsole -debug=ipc
BITCOIND=$(pwd)/build/bin/runebase-node build/test/functional/test_runner.py
```
...
[#19460](https://github.com/bitcoin/bitcoin/pull/19460) also adds a new `runebase-node` `-ipcbind` option and a `runebased-wallet` `-ipcconnect` option to allow new wallet processes to connect to an existing node process.
