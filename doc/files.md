...
The data directory is the default location where the Runebase Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.runebase/`
macOS    | `$HOME/Library/Application Support/Runebase/`
Windows  | `%LOCALAPPDATA%\Runebase\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `runebase.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Runebase blocks (dumped in network format, 128 MiB per file)
...
`./`               | `runebase.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `runebased` or `runebase-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `runebased.pid`        | Stores the process ID (PID) of `runebased` or `runebase-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `runebased` or `runebase-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [runebase.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`runebase-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Runebase Core:
...
This table describes the files installed by Runebase Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| runebase.conf                                               | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/runebase                                                | Command-line tool for interacting with Runebase. Calls other binaries below. |
| bin/runebase-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/runebase-qt                                             | Runebase node and wallet GUI                                                 |
| bin/runebase-tx                                             | Tool for creating and modifying transactions                                |
| bin/runebase-util                                           | Miscellaneous utilities                                                     |
| bin/runebase-wallet                                         | Runebase wallet tool                                                         |
| bin/runebased                                               | Runebase node and wallet daemon                                              |
...
| *libexec/bench_runebase*                                    | Benchmarking tool for measuring node performance                            |
| *libexec/runebase-chainstate*                               | Tool to validate and connect blocks                                         |
| libexec/runebase-gui                                        | IPC-enabled alternative to `runebase-qt`                                     |
| libexec/runebase-node                                       | IPC-enabled alternative to `runebased`                                       |
| libexec/test_runebase                                       | Unit test binary                                                            |
| *libexec/test_runebase-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `runebase-cli`, `runebased`, and others |
...
- README and runebase.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `runebase-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Runebase Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
