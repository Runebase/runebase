...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'runebase.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "runebase.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        bad_conf_file_path = self.nodes[0].datadir_path / "runebase_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'runebase.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "runebase_main.conf"
    ...
    def test_config_file_log(self):
        ...
        conf_path = default_datadir / "runebase.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a runebase.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "runebase.conf" file which is ignored, because a '
        ...
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/runebase.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when runebase.conf in the default data directory sets another datadir '
                      'and it contains a different runebase.conf file that would be ignored')
        ...
        node = self.nodes[0]
        (default_datadir / "runebase.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "runebase.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/runebase.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "runebase.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "runebase.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_runebase.conf"
