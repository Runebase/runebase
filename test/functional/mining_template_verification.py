...
class MiningTemplateVerificationTest(BitcoinTestFramework):
    ...
    def block_without_transactions_test(self, node, block):
        ...
        no_tx_block.solve()
        # Runebase returns "bad-cb-missing" instead of Bitcoin's "bad-blk-length"
        # because the block size limit check (bad-blk-length) was moved from
        # CheckBlock to ConnectBlock in Runebase (validation.cpp:3474), since it
    ...
    def bad_timestamp_test(self, node, block):
        ...
        # Runebase only enforces time-too-new/time-too-old for PoS blocks, not PoW
