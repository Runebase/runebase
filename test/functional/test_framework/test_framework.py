...
from .runebaseconfig import COINBASE_MATURITY
from .runebase import generatesynchronized
...
TMPDIR_PREFIX = "runebase_func_test_"
...
class Binaries:
    """Helper class to provide information about runebase binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke runebased"
        return self._argv("node", self.paths.bitcoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke runebase-cli"
        # Add -nonamed because "runebase rpc" enables -named by default, but runebase-cli doesn't
        return self._argv("rpc", self.paths.bitcoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke runebase-tx"
        return self._argv("tx", self.paths.bitcointx)

    def util_argv(self):
        "Return argv array that should be used to invoke runebase-util"
        return self._argv("util", self.paths.bitcoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke runebase-wallet"
        return self._argv("wallet", self.paths.bitcoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke runebase-chainstate"
    ...
    def _argv(self, command, bin_path, need_ipc=False):
        ...
        either uses the runebase wrapper executable (if BITCOIN_CMD is set or
        need_ipc is True), or the direct binary path (runebased, etc). When
        ...
        if self.bin_dir is not None:
            ...
            # If the current test needs IPC functionality, use the runebase
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a runebase test script.

    Individual runebase test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave runebaseds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use runebase-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "runebase": "BITCOIN_BIN",
            "runebased": "BITCOIND",
            "runebase-cli": "BITCOINCLI",
            "runebase-util": "BITCOINUTIL",
            "runebase-tx": "BITCOINTX",
            "runebase-chainstate": "BITCOINCHAINSTATE",
            "runebase-wallet": "BITCOINWALLET",
        }
        ...
        # BITCOIN_CMD environment variable can be specified to invoke runebase
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a runebased test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple runebased test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the runebased
    ...
    def _start_logging(self):
        ...
        # Format logs the same as runebased's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            for entry in os.listdir(cache_path()):
                if entry not in ['chainstate', 'blocks', 'indexes', 'stateRunebase']:  # Only indexes, chainstate and blocks folders
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if runebased has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("runebased has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if runebased has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("runebased has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if runebase-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("runebase-wallet has not been compiled")

    def skip_if_no_bitcoin_tx(self):
        """Skip the running test if runebase-tx has not been compiled."""
        if not self.is_bitcoin_tx_compiled():
            raise SkipTest("runebase-tx has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if runebase-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("runebase-util has not been compiled")

    def skip_if_no_bitcoin_chainstate(self):
        """Skip the running test if runebase-chainstate has not been compiled."""
        if not self.is_bitcoin_chainstate_compiled():
            raise SkipTest("runebase-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if runebase-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("runebase-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether runebase-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether runebase-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_tx_compiled(self):
        """Checks whether runebase-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_bitcoin_util_compiled(self):
        """Checks whether runebase-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_bitcoin_chainstate_compiled(self):
        """Checks whether runebase-chainstate was compiled."""
