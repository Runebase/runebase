...
def convert_btc_address_to_runebase(addr, main=False):
...
def convert_btc_bech32_address_to_runebase(addr, main=False, encoding=Encoding.BECH32):
    ...
    return bech32_encode(encoding, 'rcrt' if not main else 'rc', data)
...
class DGPState:
    ...
    def send_set_initial_admin(self, sender):
        ...
        self.node.sendtocontract(self.contract_address, self.abiSetInitialAdmin, 0, 2000000, RUNEBASE_MIN_GAS_PRICE_STR, sender)
