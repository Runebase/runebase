...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Ensure runebase node command invokes runebased by default")
        self.test_args([], [], expect_exe="runebased")

        self.log.info("Ensure runebase -M invokes runebased")
        self.test_args(["-M"], [], expect_exe="runebased")

        self.log.info("Ensure runebase -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure runebase -m invokes runebase-node")
            self.test_args(["-m"], [], expect_exe="runebase-node")

            self.log.info("Ensure runebase -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="runebase-node")

            self.log.info("Ensure runebase accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="runebase-node")

            self.log.info("Ensure runebase recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="runebase-node")
