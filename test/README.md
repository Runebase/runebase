This directory contains integration tests that test runebased and its
...
runebased and runebase-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (runebase-util, runebase-tx, ...).
...
Before tests can be run locally, Runebase Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the runebased nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another runebased
process running on the system (perhaps from a previous test which hasn't successfully
killed all its runebased nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other runebased processes are running.

On linux, the test framework will warn if there is another
runebased process running when the tests are started.

If there are zombie runebased processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
runebased processes running on the system, so should not be used if any non-test
runebased processes are being run.**
...
sure runebased processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and runebased `debug.log`s will all be dumped
...
`test_framework.log` and runebased `debug.log`s can be combined into a single
...
call methods that interact with the runebased nodes-under-test.

If further introspection of the runebased instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/runebased.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/runebased <pid>
```
